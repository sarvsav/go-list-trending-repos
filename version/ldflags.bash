...
if [ -n "$tag" ]; then
  ...
  echo -n " -X github.com/sarvsav/go-list-trending-repos/version.tag=$tag "
else
  # No exact tag on this commit; get the latest tag and the current commit hash
  last_tag=$(git describe --tags --abbrev=0)
  commit_hash=$(git rev-parse --short HEAD)
  echo -n " -X github.com/sarvsav/go-list-trending-repos/version.tag=$last_tag-$commit_hash "
fi

# Add commit hash and date information
echo -n $(git show -s --format=' -X github.com/sarvsav/go-list-trending-repos/version.commit=%H -X github.com/sarvsav/go-list-trending-repos/version.date=%ct')
